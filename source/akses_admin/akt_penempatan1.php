...
if ($_REQUEST['keahkod'] != "")
	{
	//cek, sudah ada belum
	$SQL1 = sprintf("SELECT * FROM siswa_keahlian ".
	...
	//jika sudah ada, update aja keahlian e
	if ($totalRows_rs1 != 0) 
		{
		//update-kan baru
		mysql_select_db($database_sisfokol, $sisfokol);
		$query_rsupdate = "UPDATE siswa_keahlian SET kd_keahlian = '$keahkod' ".
		...
		$returner = "akt_penempatan.php?jurikod=$jurikod&kelikod=$kelikod";
...
if ($_REQUEST['kelkod'] != "")
	{
	//cek, sudah ada belum
	$SQL1 = sprintf("SELECT * FROM siswa_tingkat ".
						"WHERE kd_tapel = '$inc_tapel' ".
						"AND kd_tingkat = '$kelkod' ".
						"AND kd_siswa = '$kd_siswa'");
	...
	//jika sudah ada, update aja tingkat e
	if ($totalRows_rs1 != 0) 
		{
		//netralkan dahulu semuanya
		mysql_select_db($database_sisfokol, $sisfokol);
		$query_rsnetral = "UPDATE siswa_tingkat SET status = 'false' WHERE kd_siswa = '$kd_siswa'";
		$rsnetral = mysql_query($query_rsnetral, $sisfokol) or die(mysql_error());		
		
		//update-kan baru
		mysql_select_db($database_sisfokol, $sisfokol);
		$query_rsupdate = "UPDATE siswa_tingkat SET status = 'true' ".
							"WHERE kd_tingkat = '$kelkod' ".
							"AND kd_siswa = '$kd_siswa'";
		$rsupdate = mysql_query($query_rsupdate, $sisfokol) or die(mysql_error());
	...
	//belum ada
		...
		{
		mysql_select_db($database_sisfokol, $sisfokol);
		$query_rs_tapel1 = "INSERT INTO siswa_tingkat(kd_tapel, kd_tingkat, kd_siswa, status)".
